...
def _route_result_to_dict(route_result) -> Dict:
    return {
        "route_index": route_result.route_index,
        "path": list(route_result.path),
        "feasible": bool(route_result.feasible),
        "distance": float(route_result.distance),
        "total_time": float(route_result.total_time),
        "load": float(route_result.load),
        "waiting_time": float(route_result.waiting_time),
        "errors": list(route_result.errors),
...
def run_experiments(config: ExperimentConfig) -> ExperimentSummary:
    ...
    output_dir = Path(config.output_dir) / "instances_json"
    ...
    for idx, instance in enumerate(instances, 1):
        ...
        output_path = output_dir / f"{instance.name}.json"
        write_instance_json(
